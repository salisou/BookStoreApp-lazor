namespace bookstoreApp.Api.Controllers
{
    ...
    public class AuthorsController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<AuthorReadOnlyDto>>> GetAuthors()
        {
            ...
            {
                logger.LogError(ex, $"Error performance Get in {nameof(GetAuthors)}");
        ...
        public async Task<ActionResult<AuthorReadOnlyDto>> GetAuthor(int id)
        {
            try
            {
                ...
                if (author == null)
                {
                    logger.LogWarning($"{Messages.RecodNotFound}, {nameof(GetAuthor)} - ID: {id}");
                    return NotFound();
                }
        ...
        public async Task<IActionResult> PutAuthor(int id, AuthorUpdateDto authorDto)
        {
            if (id != authorDto.Id)
            {
                logger.LogWarning($"Update invalide in, {nameof(PutAuthor)} - ID: {id}");
                return BadRequest();
            }
        ...
        public async Task<ActionResult<AuthorCreateDto>> PostAuthor(AuthorCreateDto authorDto)
        {
            ...
            {
                logger.LogError(ex, $"Error performance Post in {nameof(PostAuthor)}");
                return StatusCode(500, Messages.Error500Message);
            }
        ...
        public async Task<IActionResult> DeleteAuthor(int id)
        {
            ...
            {
                logger.LogError(ex, $"Error performance Delete in {nameof(DeleteAuthor)}");
                return StatusCode(500, Messages.Error500Message);
            }
        }
